...
export default function Geral() {
  const [anoSelecionado, setAnoSelecionado] = useState<string | undefined>(undefined);
  const { dados, rankingConcorridos, metricas } = useMetricasGerais(anoSelecionado);
  const {
    totalCandidatos,
    mediaNotaCandidato,
    mediaNotaCorte,
  ...
  return (
    <Container sx={{ paddingTop: 4 }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
        ...
        <ToggleButtonGroup
          value={anoSelecionado ?? "todos"}
          exclusive
          onChange={(_, val: string) => setAnoSelecionado(val === "todos" ? undefined : val)}
          size="small"
        >
          <ToggleButton value="todos">Todos</ToggleButton>
          {anos.map((ano) => (
            <ToggleButton key={ano} value={ano}>
              {ano}
            </ToggleButton>
          ))}
        </ToggleButtonGroup>
      ...
      <CandidatosBarChart ano={anoSelecionado} />
